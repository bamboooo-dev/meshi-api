...
func (lq *LikeQuery) QueryRestaurant() *RestaurantQuery {
	query := &RestaurantQuery{config: lq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, like.RestaurantTable, like.RestaurantColumn),
		)
...
func (lq *LikeQuery) Clone() *LikeQuery {
	...
	return &LikeQuery{
		...
		withRestaurant: lq.withRestaurant.Clone(),
...
func (lq *LikeQuery) WithRestaurant(opts ...func(*RestaurantQuery)) *LikeQuery {
	query := &RestaurantQuery{config: lq.config}
	for _, opt := range opts {
		opt(query)
	}
	lq.withRestaurant = query
	return lq
}
...
//		Scan(ctx, &v)
//
func (lq *LikeQuery) Select(field string, fields ...string) *LikeSelect {
...
func (lq *LikeQuery) sqlAll(ctx context.Context) ([]*Like, error) {
	var (
		...
		_spec       = lq.querySpec()
		loadedTypes = [1]bool{
			lq.withRestaurant != nil,
		}
	)
	if lq.withRestaurant != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := lq.withRestaurant; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "restaurant_likes" returned %v`, n.ID)
			}
			for i := range nodes {
